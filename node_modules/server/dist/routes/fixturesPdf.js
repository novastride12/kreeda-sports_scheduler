...
router.get('/', async (req, res) => {
    try {
        ...
        const tournament = await Tournament_1.Tournament.findById(id).populate('champions.team');
        if (!tournament) {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Kreeda_${tournament.name.replace(/\s+/g, '_')}_Fixtures.pdf"`);
